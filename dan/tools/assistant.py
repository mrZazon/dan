...
class CalendarListTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            message=f"Calendar ({len(filtered)} events):\n" + "\n".join(lines),
        )
...
class ReminderSetTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        remind_at = kwargs.get("time", kwargs.get("datetime", ""))
...
class ReminderListTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            message=f"Reminders ({len(active)} active):\n" + "\n".join(lines),
        )
...
class DailyBriefingTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        lines = [f"Good morning! Today is {day_name}, {today}. It's {time_str}.\n"]
